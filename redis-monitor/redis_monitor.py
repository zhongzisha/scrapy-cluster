...
class RedisMonitor(object):
    ...
    def setup(self, level=None, log_file=None, log_use_json=None):
        ...
        self.logger = LogFactory.get_instance(use_json=my_json,
    ...
    def _load_plugins(self):
        ...
        for key in plugins:
            ...
            self.logger.info("Trying to load plugin {cls}"
                             .format(cls=key))
    ...
    def _process_failures(self, key):
        ...
        if self.settings['RETRY_FAILURES']:
            self.logger.info("going to retry failure")
            ...
            if current < self.settings['RETRY_FAILURES_MAX']:
                self.logger.info("Incr fail key")
                current += 1
