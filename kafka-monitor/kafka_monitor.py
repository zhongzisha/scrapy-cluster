...
class KafkaMonitor(object):
    ...
    def setup(self, log_level=None, log_file=None, log_use_json=None):
        '''
        ...
        @param log_use_json: boolean t/f whether to write the logs in use_json
        '''

        self.settings = self.wrapper.load(self.settings_name)

        my_level = log_level if log_level else self.settings['LOG_LEVEL']
        # negate because logger wants True for std out
        my_output = not log_file if log_file else self.settings['LOG_STDOUT']
        my_use_json = log_use_json if log_use_json else self.settings['LOG_USE_JSON']
        self.logger = LogFactory.get_instance(use_json=my_use_json, stdout=my_output,
                                              level=my_level,
                ...
                try:
                    ...
                    for key in self.plugins_dict:
                        ...
                        try:
                            ...
                            pass
                    if not found_plugin:
                        extras = {'parsed': True, 'valid': False, 'data': the_dict}
    ...
    def _increment_fail_stat(self, item):
        ...
        if isinstance(item, dict):
            item['ts'] = time.time()
            item_string = json.dumps(item).encode('utf-8')
        elif isinstance(item, str):
            item_string = item + str(time.time())

        if 'fail' in self.stats_dict:
            self.logger.info("Incremented fail stats")
            for key in self.stats_dict['fail']:
                if key == 'lifetime':
                    self.stats_dict['fail'][key].increment(item_string)
                else:
    ...
    def _increment_plugin_stat(self, name, item):
        ...
        if 'plugins' in self.stats_dict:
            ...
            for key in self.stats_dict['plugins'][name]:
                if key == 'lifetime':
                    self.stats_dict['plugins'][name][key].increment(item_string)
                else:
    ...
    def feed(self, json_item):
        ...
        def _feed(json_item):
            ...
            if producer is not None:
                producer.send(topic, value=json_item)
                producer.flush()
...
def main():
    ...
    base_parser.add_argument('-lj', '--log-use-json', action='store_const',
                             required=False, const=True, default=None,
    ...
    kafka_monitor.setup(log_level=args['log_level'], log_file=args['log_file'],
