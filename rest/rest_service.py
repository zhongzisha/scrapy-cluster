...
def validate_json(f):
    ...
    def wrapper(*args, **kw):
        ...
        try:
            ...
            instance.logger.error(instance.MUST_JSON)
            return jsonify(ret_dict), 400
        instance.logger.info("JSON is valid")
        return f(*args, **kw)
    ...
    def decorator(f):
        ...
        def wrapper(*args, **kw):
            ...
            try:
                ...
                return jsonify(ret_dict), 400
            instance.logger.info("Schema is valid")
            return f(*args, **kw)
...
class RestService(object):
    ...
    def setup(self, level=None, log_file=None, log_use_json=None):
        """
        ...
        my_json = log_use_json if log_use_json else self.settings['LOG_USE_JSON']
        self.logger = LogFactory.get_instance(use_json=my_json, stdout=my_output,
                                              level=my_level,
        ...
        for filename in os.listdir(self.settings['SCHEMA_DIR']):
            if filename[-4:] == 'json':
                ...
                with open(self.settings['SCHEMA_DIR'] + filename) as the_file:
                    ...
                    self.logger.info("Successfully loaded " + filename + " schema")
    ...
    def _spawn_redis_connection_thread(self):
        ...
        self.logger.info("Spawn redis connection thread")
        self.redis_connected = False
    ...
    def _spawn_kafka_connection_thread(self):
        ...
        self.logger.info("Spawn kafka connection thread")
        self.kafka_connected = False
    ...
    def _spawn_kafka_consumer_thread(self):
        ...
        self.logger.info("Spawn kafka consumer thread""")
    ...
    def _consumer_loop(self):
        ...
        self.logger.info("running main consumer thread")
    ...
    def _send_result_to_redis(self, result):
        ...
        if self.redis_connected:
            self.logger.info("Sending result to redis")
            try:
    ...
    def _heartbeat_loop(self):
        ...
        self.logger.info("running main heartbeat thread")
        while not self.closed:
    ...
    def _report_self(self):
        ...
        """
        if self.redis_connected:
            self.logger.info("Reporting self to redis")
            try:
    ...
    def _setup_redis(self):
        ...
        if not self.closed:
            try:
                self.logger.info("Creating redis connection to host " +
    ...
    def _setup_kafka(self):
        ...
        if self.consumer is not None:
            self.logger.info("Closing existing kafka consumer")
            self.consumer.close()
            self.consumer = None
        if self.producer is not None:
            self.logger.info("Closing existing kafka producer")
            self.producer.flush()
        ...
        self.logger.info("Creating kafka connections")
        self.consumer = self._create_consumer()
        if not self.closed:
            self.logger.info("Kafka Conumer created")
    ...
    def _close_thread(self, thread, thread_name):
        ...
        if thread is not None and thread.isAlive():
            self.logger.info("Waiting for {} thread to close".format(thread_name))
    ...
    def _kafka_success(self, response):
        ...
        '''
        self.logger.info("Sent message to Kafka")
    ...
    def _decorate_routes(self):
        ...
        """
        self.logger.info("Decorating routes")
        ...
        if self.kafka_connected:
            ...
            if result:
                ...
                if self.wait_for_response:
                    self.logger.info("expecting kafka response for request")
                    ...
                    while not found_item and int(self.get_time() - the_time) <= self.settings['WAIT_FOR_RESPONSE_TIME']:
                        ...
                        with self.uuids_lock:
                            ...
                            if json_item['uuid'] in self.uuids:
                                ...
                                self.logger.info("Did not find response, "
                                                 "adding to poll")
                    if found_item:
                        self.logger.info("Got successful reponse back from kafka")
                    else:
                    ...
                    self.logger.info("Not expecting response from kafka")
        ...
        if self.redis_connected:
            ...
            try:
                ...
                if result is not None:
                    ...
                    self.logger.info("Found previous poll")
                    self.redis_conn.delete(key)
                    return self._create_ret_object(self.SUCCESS, result)
                else:
                    self.logger.info("poll key does not exist")
                    return self._create_ret_object(self.FAILURE, None, True,
...
if __name__ == '__main__':
    ...
    parser.add_argument('-lj', '--log-use-json', action='store_const',
                        required=False, const=True, default=None,
    ...
    rest_service.setup(level=args['log_level'], log_file=args['log_file'],
                       log_use_json=args['log_use_json'])
