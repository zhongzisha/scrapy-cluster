...
::
    ...
    class MyMonitor(BaseMonitor):
        ...
        def check_precondition(cls, key, value):
            '''
            Override if special conditions need to be met
            '''
            pass

        def handle(cls, key, value):
            '''
...
::
    ...
    class MyMonitor(KafkaBaseMonitor):
        ...
        def setup(cls, settings):
            ...
            Setup kafka
            '''
            KafkaBaseMonitor.setup(cls, settings)
        ...
        def handle(cls, key, value):
            ...
            # ...
            # now, send to kafka
            if cls._send_to_kafka(results):
                cls.logger.info('Sent results to kafka')
            else:
                cls.logger.error('Failed to send results to kafka')
...
Every Redis Monitor plugin is provided a Scrapy Cluster logger, under the variable name ``cls.logger``. You can use this logger to generate debug, info, warnings, or any other log output you need to help gather information from your plugin. This is the same logger that the core Redis Monitor uses, so your desired settings will be preserved.

Each Plugin is also provided a default Redis Connection variable, named ``cls.redis_conn``. This variable is an instance of a Redis Connection thanks to the `redis-py <https://redis-py.readthedocs.org>`_ library and can be used to manipulate anything within your Redis instance.
